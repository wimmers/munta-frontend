...
let make =
  ...
  render: ({reduce, state, handle}) => {
    ...
    let disabled_class =
      "btn btn-default" ++ (selected == None ? " disabled" : "");
    <div>
      ...
      <div className="form-group row" id="item-list">
        ...
        <div className="col-sm-3 btn-group btn-group-md" role="group">
          ...
          <button className=disabled_class onClick=(on_selected(onCopy))>
            <span className="glyphicon glyphicon-duplicate" />
            <span className="sr-only"> (str("Copy")) </span>
          </button>
          <button className=disabled_class onClick=(on_selected(onDelete))>
